  ...
  updateVisuals() {
    ...
    const content =
      ...
      (this.campaignUrlProp === 'https://kaup.ee' || this.campaignUrlProp === 'https://kaup24.ee')
        ...
        ? `*Скидочный код 15WKND1124 в силе c 22.11.2024 по 24.11.2024 до 23:59 для зарегистрированных пользователей, при покупке особо отмеченных товаров продавца Kaup24.ee. Скидочный код не действует при покупке подарочных карт и вместе с Kaup24-eвро и с другим скидочным кодом. Для одной покупки можно использовать только один скидочный код.
`
        : this.language === 'EN' &&
          (this.campaignUrlProp === 'https://kaup.ee' ||
            this.campaignUrlProp === 'https://kaup24.ee')
        ? `*Discount code 15WKND1124 is valid for specially marked seller Kaup24.ee items from 22.11.2024 until 24.11.2024 till 23:59, for register users. The discount code is not valid when purchasing gift cards and together with Kaup24.ee money or with other codes. Only one discount code can be used per purchase.`
        : this.language === 'LT' && this.campaignUrlProp === 'https://pigu.lt'
        ? `*Nuolaidos kodas 15WKND1124 galioja iki 2024 11 24 iki 24 val. registruotiems vartotojams, perkantiems spec. ženklu pažymėtų pardavėjo Pigu.lt prekių. Nuolaidos kodas negalioja dovanų kuponams ir negalioja kartu su PiguEurais. Vieno pirkimo metu galima panaudoti tik vieną nuolaidos kodą.`
        : this.language === 'RU' && this.campaignUrlProp === 'https://pigu.lt'
        ? `*Скидочный код 15WKND1124 в силе до 24.00, 24.11.2024 для зарегистрированных пользователей при покупке особо отмеченных товаров продавца Pigu.lt. Скидочный код не действует при покупке подарочных карт и вместе с Pigu-Евро. Для одной покупки можно использовать только один скидочный код.`
        : this.language === 'FI' && this.campaignUrlProp === 'https://hobbyhall.fi'
        ? `*Alekoodi 15VKLP1124 on voimassa 22.-24.11.2024 asti, valikoiduista ja merkityistä myyjän Hobbyhall.fi-tuotteista. Voimassa vain rekisteröityneille asiakkaille. Ei voi yhdistää muihin alekoodeihin, Hobby Hall rahaan tai lahjakorttia ostaessa.`
        : this.language === 'LV' && this.campaignUrlProp === 'https://220.lv'
        ? `*Atlaižu kods 15WKND1124 ir spēkā no 22.11.2024 līdz 24.11.2024. līdz plkst. 23:59 reģistrētiem lietotājiem, kuri iegādāsies īpaši marķētas pārdevēja 220.lv preces. Atlaižu kods nav derīgs, iegādājoties dāvanu kartes un kopā ar 220.lv naudu. Vienam pirkumam var izmantot tikai vienu atlaižu kodu.`
        ...
        ? `*Discount code 15WKND1124 is valid for specially marked seller Pigu.lt items from 22.11.2024 until 24.11.2024 till 23:59, for register users. The discount code is not valid when purchasing gift cards and together with Pigu.lt money or with other codes. Only one discount code can be used per purchase.`
        : this.language === 'EN' && this.campaignUrlProp === 'https://hobbyhall.fi'
        ? `*Discount code 15WKND1124 is valid for specially marked seller Hobby Hall items from 22.11.2024 until 24.11.2024 till 23:59, for register users. The discount code is not valid when purchasing gift cards and together with Hobby Hall money or with other codes. Only one discount code can be used per purchase.`
        : this.language === 'EN' &&
          this.campaignUrlProp === 'https://220.lv' &&
          `*Discount code 15WKND1124 is valid for specially marked seller 220.lv items from 22.11.2024 until 24.11.2024 till 23:59, for register users. The discount code is not valid when purchasing gift cards and together with 220.lv money or with other codes. Only one discount code can be used per purchase.`;
