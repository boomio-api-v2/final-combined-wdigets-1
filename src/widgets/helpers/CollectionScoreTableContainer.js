...
export class CollectionScoreTableContainer {
  ...
  updateVisuals() {
    ...
    let fontSize = this.prop === 'Barbora' || this.prop === 'Fpro' || this.prop === 'Fantazijos' || this.customer.includes('Gamtos Ateitis') || this.prop === 'LemonGym' ? '14px' : '10px';
    let fontWeight = this.prop === 'Barbora' || this.prop === 'Fpro' || this.prop === 'Fantazijos' || this.customer.includes('Gamtos Ateitis') || this.prop === 'LemonGym' ? '900' : '700';
    let scoreboardText = `
      ${
        this.prop === 'Barbora' || this.prop === 'Fpro' || this.prop === 'Fantazijos' || this.customer.includes('Gamtos Ateitis') || this.prop === 'LemonGym'
          ? this.collectables?.user_best_place < (this.prop === 'Barbora' ? 0 : this.prop === 'LemonGym' ? 11 : 30) || this.prop === 'Barbora' || this.prop === 'Fantazijos'
            ? `<div style="width:100%; top: ${'440px'}; position: absolute; text-align: center; color: white; font-size: ${
                ...
                this.prop === 'Barbora' ? 'DOVANA tau!' : this.prop === 'Fantazijos' ? '2024.06.09 ŠVENČIANT NACIONALINĘ 69 DIENĄ' : 'Valio, tau puikiai sekasi!'
              }</div>
            ...
            <div style="width:100%; top: 546px; position: absolute; text-align: center; color: white; font-size: 10px; font-family: Montserrat; font-weight: 700; text-transform: uppercase; word-wrap: break-word">${
              this.prop === 'Barbora' ? '(Galioja pristatymams iki 04 14 d.)' : this.prop === 'Fantazijos' ? 'GAUK 19% NUOLAIDĄ VISKAM!' : 'Apie laimėjimą informuosime nurodytu el. paštu.'
            } </div> `
            : `<div style="width:100%; top: 440px; position: absolute; text-align: center; color: white; font-size: ${fontSize}; font-family: Montserrat; font-weight: ${fontWeight}; text-transform: uppercase; word-wrap: break-word">${
                this.prop === 'Barbora' ? 'Pagerink rezultatą ir laimėk </br>Barbora gimtadienio dovaną iškart!' : 'Tu gali!'
              }</div>
  ...
  render() {
    ...
    containerDiv.style.width = document.documentElement.clientWidth < 426 ? (document.documentElement.clientWidth < 321 ? '375px' : document.documentElement.clientWidth + 'px') : '380px';
