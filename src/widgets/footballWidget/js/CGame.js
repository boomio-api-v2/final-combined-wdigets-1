function CGame(oData) {
  ...
  this.ballPosition = function () {
    ...
    var fScaleDistance =
      oPos2DBall.z * (BALL_SCALE_FACTOR - _oBall.getStartScale()) + _oBall.getStartScale();
  ...
  this.predictBallGoalPos = function (pDirection) {
    ...
    var iFinalX = linearFunction(
      iNormalizedX,
      STRIKER_GOAL_SHOOTAREA.lx,
      STRIKER_GOAL_SHOOTAREA.rx,
      -_pGoalSize.w / 2,
      _pGoalSize.w / 2,
    );
    ...
    var iFinalY =
      (-_pGoalSize.h / Math.pow(STRIKER_GOAL_SHOOTAREA.zmax, 2)) * iNormalizedY * iNormalizedY +
      _pGoalSize.h / 2;
  ...
  this.displayShock = function (iTime, iXIntensity, iYIntensity) {
    ...
    createjs.Tween.get(_oContainerGame)
      .to(
        { x: Math.round(Math.random() * xShifting), y: Math.round(Math.random() * yShifting) },
        iTime,
      )
      .call(function () {
        createjs.Tween.get(_oContainerGame)
