function CHandSwipeAnim(oStartPoint, aEndPoint, oSprite, oParentContainer) {
  ...
  this.animAllSwipe = function () {
    ...
    _oHandSwipe.x = _oStartPoint.x;
    _oHandSwipe.y = _oStartPoint.y;
    createjs.Tween.get(_oHandSwipe, { loop: -1 })
      .to(
        { x: _aEndPoint[0].x, y: _aEndPoint[0].y, alpha: 1 },
        MS_TIME_SWIPE_END,
        createjs.Ease.quartOut,
      )
      .to({ x: _oStartPoint.x, y: _oStartPoint.y, alpha: 0.5 }, 0)
      .to(
        { x: _aEndPoint[1].x, y: _aEndPoint[1].y, alpha: 1 },
        MS_TIME_SWIPE_END,
        createjs.Ease.quartOut,
      )
      .to({ x: _oStartPoint.x, y: _oStartPoint.y, alpha: 0.5 }, 0)
      .to(
        { x: _aEndPoint[2].x, y: _aEndPoint[2].y, alpha: 1 },
        MS_TIME_SWIPE_END,
        createjs.Ease.quartOut,
      )
      .to({ x: _oStartPoint.x, y: _oStartPoint.y, alpha: 0.5 }, 0);
